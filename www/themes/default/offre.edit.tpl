...
<form id="form" name="form" method="POST" class="offreForm" action='#'>
	...
	{% endif %}
				...
				<table id='attachedFiles'>
					{% for d in documents %}
						<tr><td><a href='{{webRoot}}docs/offres/{{offre.offreId}}/{{d.url}}' target='_blank'>{{d.url}}</a></td><td>{% if d.type=='image' %}<img src="{{webRoot}}docs/offres/{{offre.offreId}}/{{d.url}}" style="height:60px"/>{% endif %}</td></tr>
					{% endfor %}
					<tr><td colspan='2'><input id="file_upload" name="file_upload" type="file"  /><input type='hidden' id='attachedFilesFolder' value='docs/offres/{{offre.offreId}}' /></td></tr>
				</table>
