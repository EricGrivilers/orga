...
class Client
{
    ...
    */
    ...
    public function __construct()
    {
     ...
     public function validateClientType(ExecutionContextInterface $ec)
  {
    if ($this->clienttype=='cie')
    {
      //$ec->getGraphWalker()->walkReference($this, 'is_cie_group', $ec->getPropertyPath(), true);
        ...
        if($this->name=='') {
            $ec
            ->buildViolation('Cannot be blank.') // message
            ->atPath('is_cie_group')                                                   // attribut de l'objet qui est violé
            ->addViolation();
        }
        ...
        if($this->lastname=='') {
            $ec
            ->buildViolation('Cannot be blank.') // message
            ->atPath('is_part_group')                                                   // attribut de l'objet qui est violé
            ->addViolation();
        }
    }
  }
